...
impl GitCommandExecutor {
  ...
  /// Execute a git command and process output through a callback
  /// This is more memory efficient for commands that produce large outputs
  #[instrument(
    skip(self, output_processor),
    fields(
      git_command = args.join(" "),
      repository_path = repository_path,
      success = tracing::field::Empty,
  ...
  pub fn execute_command_streaming<F>(&self, args: &[&str], repository_path: &str, mut output_processor: F) -> Result<()>
  where
    F: FnMut(&[u8]) -> Result<()>,
  {
    if repository_path.is_empty() {
    ...
    // Read and process output in chunks
    use std::io::Read;
    ...
    loop {
      match reader.read(&mut buffer) {
        Ok(0) => break, // EOF
        Ok(n) => {
          output_processor(&buffer[..n])?;
        }
        Err(e) if e.kind() == std::io::ErrorKind::Interrupted => continue,
        Err(e) => return Err(anyhow!("Failed to read from git output: {e}")),
      }
    }

    // Wait for the process to complete
    let status = child.wait().map_err(|e| anyhow!("Failed to wait for git command: {e}"))?;
    ...
    if status.success() {
      ...
      if let Some(mut stderr_reader) = child.stderr {
        use std::io::Read;
        stderr_reader.read_to_string(&mut stderr).ok();
      }
      tracing::Span::current().record("success", false);
      tracing::error!(stderr = %stderr, "git command failed");
      Err(anyhow!("git command failed: {} {}\nError: {}", git_info.path, args.join(" "), stderr.trim()))
    }
  }
}
