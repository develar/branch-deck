...
pub fn load_issue_navigation_config(repository_path: &str) -> Option<IssueNavigationConfig> {
  ...
  if !vcs_xml_path.exists() {
    debug!("No .idea/vcs.xml found at {:?}", vcs_xml_path);
    return None;
  }
  ...
  let xml_content = match std::fs::read_to_string(&vcs_xml_path) {
    Ok(content) => content,
    Err(e) => {
      warn!("Failed to read .idea/vcs.xml: {}", e);
      return None;
    }
  };
...
fn parse_issue_navigation_xml(xml: &str) -> Option<IssueNavigationConfig> {
  ...
  loop {
    match reader.read_event_into(&mut buf) {
      ...
      Ok(Event::Eof) => break,
      Err(e) => {
        warn!("Error parsing .idea/vcs.xml: {}", e);
        break;
      }
      _ => {}
    }
  ...
  if links.is_empty() {
    ...
    debug!("Found {} issue navigation links", links.len());
