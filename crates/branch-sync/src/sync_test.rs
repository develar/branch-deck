...
fn test_group_commits_by_prefix() {
  ...
  assert_eq!(feature_auth[0].stripped_subject, "Add authentication");
  assert_eq!(feature_auth[1].stripped_subject, "Improve auth validation");
  assert_eq!(feature_auth[2].stripped_subject, "Add two-factor auth");

  // Check bugfix-login group has 1 commit
  let bugfix_login = grouped.get("bugfix-login").unwrap();
  assert_eq!(bugfix_login.len(), 1);
  assert_eq!(bugfix_login[0].stripped_subject, "Fix login timeout");
...
fn test_group_commits_by_prefix_preserves_order() {
  ...
  let feature_auth = grouped.get("feature-auth").unwrap();
  assert_eq!(feature_auth.len(), 3);
  assert_eq!(feature_auth[0].stripped_subject, "First auth commit");
...
fn test_group_commits_by_prefix_handles_malformed_messages() {
  ...
  let feature_auth = grouped.get("feature-auth").unwrap();
  assert_eq!(feature_auth[0].stripped_subject, "Valid commit");
  ...
  for (i, (message, expected)) in test_cases.into_iter().enumerate() {
    ...
    match expected {
      Some((expected_prefix, expected_message)) => {
        ...
        assert_eq!(commit_list[0].stripped_subject.trim(), expected_message.trim(), "Message mismatch for message: '{message}'");
      }
      None => {
...
fn test_group_commits_with_issue_numbers() {
  ...
  // Verify the commit messages are correctly extracted
  assert_eq!(ijpl_commits[0].subject, "IJPL-163558: Fix observability of pending and running background write actions");
...
fn test_issue_numbers_with_square_brackets() {
  ...
  assert_eq!(
    ijpl_commits[0].subject,
    "[threading] IJPL-163558: Fix observability of pending and running background write actions"
  );
  assert_eq!(ijpl_commits[1].subject, "[threading] IJPL-163558: Enhance logging");
  assert_eq!(ijpl_commits[2].subject, "[database] IJPL-163558: Update schema");
...
fn test_prefix_stripping_in_sync_branches() {
  use crate::progress::{ProgressReporter, SyncEvent};
  use anyhow::Result;

  struct TestProgressReporter;

  impl ProgressReporter for TestProgressReporter {
    fn send(&self, _event: SyncEvent) -> Result<()> {
      // We don't need to track events for this test
      Ok(())
    }
  }
  ...
  let git_executor = GitCommandExecutor::new();
  let progress_reporter = TestProgressReporter;
  ...
  // Get the actual commits on the feature-auth branch using git log
  let output = std::process::Command::new("git")
    .args(["--no-pager", "log", "--reverse", "--pretty=format:%s", "origin/main..test-prefix/virtual/feature-auth"])
    .current_dir(test_repo.path())
    .output()
    .unwrap();

  assert!(output.status.success(), "git log should succeed");
  let subjects: Vec<&str> = std::str::from_utf8(&output.stdout).unwrap().lines().collect();
  ...
  let message_output = std::process::Command::new("git")
    .args(["--no-pager", "log", "-1", "--pretty=format:%B", "test-prefix/virtual/feature-auth~1"])
    .current_dir(test_repo.path())
    .output()
    .unwrap();

  let full_message = std::str::from_utf8(&message_output.stdout).unwrap();
  assert_eq!(
    full_message.trim(),
  ...
  let bugfix_output = std::process::Command::new("git")
    .args(["--no-pager", "log", "--reverse", "--pretty=format:%s", "origin/main..test-prefix/virtual/bugfix-login"])
    .current_dir(test_repo.path())
    .output()
    .unwrap();

  let bugfix_subjects: Vec<&str> = std::str::from_utf8(&bugfix_output.stdout).unwrap().lines().collect();
