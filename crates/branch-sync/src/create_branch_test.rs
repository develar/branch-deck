use crate::create_branch::{do_create_branch_from_commits, CreateBranchFromCommitsParams};
...
fn test_create_branch_from_commits_basic() {
  ...
  let result = tokio::runtime::Runtime::new()
    .unwrap()
    .block_on(do_create_branch_from_commits(&git_executor, params))
    .unwrap();
...
fn test_create_branch_with_invalid_name() {
  ...
  let result = tokio::runtime::Runtime::new().unwrap().block_on(do_create_branch_from_commits(&git_executor, params));
...
fn test_create_branch_with_missing_commits_scenario() {
  use crate::{
    progress::{ProgressReporter, SyncEvent},
    sync::sync_branches_core,
  };
  use anyhow::Result;
  ...
  let sync_result = rt.block_on(async { sync_branches_core(&git_executor, repo_path, "test-prefix", &progress_reporter).await });
  ...
  let events = progress_reporter.events.lock().unwrap();
