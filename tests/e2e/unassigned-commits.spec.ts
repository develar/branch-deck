...
import { captureAriaSnapshot, captureFloatingElementSnapshot } from "./helpers/aria-snapshot-helpers"
...
baseTest.describe("Unassigned Commits Feature", () => {
  testWithConflicts.describe("Missing Commits Detection", () => {
    ...
    testWithConflicts("should show good feature-auth branch and unassigned commits", async ({ page }) => {
      ...
      // Capture aria snapshot of unassigned commits section
      await captureAriaSnapshot(unassignedSection, "unassigned-commits-section")
    ...
    testWithConflicts("should handle branch creation with conflicts correctly", async ({ page }) => {
      ...
      // Capture aria snapshot of initial state
      await captureAriaSnapshot(unassignedSection, "unassigned-commits-before-branch-creation")
    ...
    testWithConflicts("should verify commit selection states with ARIA snapshots", async ({ page }) => {
      ...
      await captureAriaSnapshot(unassignedSection, "unassigned-commits-no-selection")
      ...
      await captureAriaSnapshot(unassignedSection, "unassigned-commits-one-selected")
      ...
      await captureAriaSnapshot(unassignedSection, "unassigned-commits-both-selected")
      ...
      await captureAriaSnapshot(floatingBar, "floating-selection-bar")
    })
  })
  ...
  baseTest.describe("Singular Form Test", () => {
    baseTest("should display singular form for 1 commit", async ({ page, setupRepo, syncAndWaitForBranches }) => {
      ...
      await captureAriaSnapshot(unassignedSection, "unassigned-commits-single")
    })
  })
})