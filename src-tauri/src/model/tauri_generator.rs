use crate::commands::suggest_branch_name::{BranchSuggestion, CommitInfo};
use anyhow::Result;
use git_ops::git_command::GitCommandExecutor;
use model_ai::generator::{ALTERNATIVE_CONFIDENCE, ModelBasedBranchGenerator as CoreGenerator, PRIMARY_CONFIDENCE};
...
impl ModelBasedBranchGenerator {
  ...
  pub async fn generate_branch_names_stream(
    ...
    commits: &[CommitInfo],
    repository_path: &str,
    progress: &tauri::ipc::Channel<crate::commands::suggest_branch_name::SuggestionProgress>,
    my_generation_id: u64,
  ) -> Result<()> {
    use crate::commands::suggest_branch_name::{BranchSuggestion, SuggestionProgress};
  ...
  pub async fn generate_branch_names(&mut self, git_executor: &GitCommandExecutor, commits: &[CommitInfo], repository_path: &str) -> Result<Vec<BranchSuggestion>> {
    // Validate commits have non-empty hashes first, before checking model
    let valid_commits: Vec<&CommitInfo> = commits.iter().filter(|c| !c.hash.is_empty()).collect();
    ...
    // Generate branch name using model-core with proven parameters
    let result = self.core.generate_branch_name(&prompt).await?;

    // Create suggestions based on the result
    let cleaned_name = clean_branch_name(&result.name)?;

    let mut suggestions = vec![BranchSuggestion {
      name: cleaned_name.clone(),
      confidence: result.confidence.max(PRIMARY_CONFIDENCE),
      reason: Some(format!("AI-generated in {}ms (confidence: {:.1}%)", result.generation_time_ms, result.confidence * 100.0)),
    }];

    // Add fallback suggestions with slight variations
    if !cleaned_name.is_empty() {
      let fallback_result = self.core.generate_alternative_branch_name(&prompt).await;
