...
pub async fn check_for_updates(app_handle: AppHandle, update_state: tauri::State<'_, SharedUpdateState>) -> Result<UpdateInfo, String> {
  ...
  match perform_update_check(&app_handle, &update_state).await {
    Ok(update_info) => {
      let mut state = update_state.lock().map_err(|e| format!("Failed to lock state: {e}"))?;
...
#[specta::specta]
#[instrument(skip(app_handle, update_state))]
pub async fn install_update(app_handle: AppHandle, update_state: tauri::State<'_, SharedUpdateState>) -> Result<(), String> {
...
#[instrument(skip(app_handle, update_state))]
async fn perform_update_check(app_handle: &AppHandle, update_state: &tauri::State<'_, SharedUpdateState>) -> anyhow::Result<UpdateInfo> {
