...
pub async fn create_branch_from_commits(git_executor: State<'_, GitCommandExecutor>, params: CreateBranchFromCommitsParams) -> Result<RewordResult, String> {
  do_create_branch_from_commits(&git_executor, params).await
}

/// Inner function that does the actual work, testable without Tauri State wrapper
pub async fn do_create_branch_from_commits(git_executor: &GitCommandExecutor, params: CreateBranchFromCommitsParams) -> Result<RewordResult, String> {
