...
pub async fn get_branch_prefix_from_git_config(git_executor: State<'_, GitCommandExecutor>, repository_path: &str) -> Result<String, String> {
  branch_sync::get_branch_prefix_from_git_config_sync(&git_executor, repository_path).map_err(|e| e.to_string())
}
...
mod tests {
  ...
  fn test_get_branch_prefix_case_insensitive() {
    ...
    let result = branch_sync::get_branch_prefix_from_git_config_sync(&git_executor, test_repo.path().to_str().unwrap()).unwrap();
  ...
  fn test_get_branch_prefix_not_found() {
    ...
    let result = branch_sync::get_branch_prefix_from_git_config_sync(&git_executor, test_repo.path().to_str().unwrap());
  ...
  fn test_get_branch_prefix_empty_path() {
    ...
    let git_executor = GitCommandExecutor::new();
    let result = branch_sync::get_branch_prefix_from_git_config_sync(&git_executor, "");
